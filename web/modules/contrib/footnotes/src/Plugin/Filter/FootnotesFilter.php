...
class FootnotesFilter extends FilterBase {
  ...
  public function process($text, $langcode) {
    ...
    // options to use.
    $this->replaceCallback($settings, 'prepare');
    ...
    if ($op == 'output footer') {
      if (count($store_matches) > 0) {
        if ($this->settings['footnotes_footer_disable']) {
          /** @var \Drupal\footnotes\FootnotesGroup $footnotes_group */
          $footnotes_group = \Drupal::service('footnotes.group');
          $footnotes_group->add($store_matches);
        }
        else {
          // Render the footnotes list.
          $markup = [
            '#theme' => 'footnote_list',
  ...
  public function settingsForm(array $form, FormStateInterface $form_state) {
    ...
    $settings['footnotes_footer_disable'] = [
      ...
      '#default_value' => $this->settings['footnotes_footer_disable'],
      '#description' => $this->t('If disabled, the footnotes will be grouped together. They can be output using the Footnotes Group block, or anywhere using Twig Tweak {{ drupal_block(\'footnotes_group\') }}.'),
    ];
    return $settings;
  }
